...
const UserProfile = ({
  ...
  const [userInputDateOfBirth, setUserInputDateOfBirth] = useState(new Date());
  ...
  const handleDateOfBirthChange = (event) => {
    setUserInputDateOfBirth(event.target.value);
  };
  ...
  const handleUpdateSurveys = () => {
    if (!newAllergies || !newMedicalHistory) {
      console.error('Allergies and Medical History are required.');
      return;
    }

    const formattedDateOfBirth = userInputDateOfBirth ? new Date(userInputDateOfBirth).toISOString().split('T')[0] : '';
